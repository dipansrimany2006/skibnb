...
const codeExamples = [
  {
    label: "Initialize",
    code: `import { Ski } from '@ski/sdk'

const ski = new Ski({
  apiKey: process.env.SKI_API_KEY
})`,
  },
  {
    label: "Inference",
    code: `const response = await ski.inference({
  model: 'ski-3-turbo',
  input: userMessage,
  stream: true
})
...
for await (const chunk of response) {
  ...
  {
    label: "Batch",
    code: `const batch = await ski.batch.create({
  model: 'ski-3',
  inputs: documents,
...
// Single-pass tokenizer — builds escaped HTML sequentially so later rules can
// never re-match markup earlier rules inserted (valid + deterministic output).
function highlightSyntax(line: string): string {
  const commentIdx = line.indexOf("//");
  const code = commentIdx >= 0 ? line.slice(0, commentIdx) : line;
  const comment = commentIdx >= 0 ? line.slice(commentIdx) : "";

  const tokenRe = /('[^']*'|"[^"]*")|\b(import|from|const|await|for|process)\b|([{}()[\]])/g;
  let html = "";
  let last = 0;
  ...
  while ((m = tokenRe.exec(code)) !== null) {
    ...
    else if (m[3]) html += `<span class="text-muted-foreground">${escapeHtml(m[3])}</span>`;
    last = tokenRe.lastIndex;
  }
  if (last < code.length) html += escapeHtml(code.slice(last));
  if (comment) html += `<span class="text-muted-foreground/50">${escapeHtml(comment)}</span>`;
  return html;
}
